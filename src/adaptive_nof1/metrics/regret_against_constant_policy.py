...
class RegretAgainstConstantPolicy(Metric):
    ...
    def score(self, simulation: Simulation):
        ...
        constant_simulation = Simulation.from_model_and_policy_with_copy(simulation.model, ConstantPolicy(number_of_actions=simulation.policy.internal_policy.number_of_actions, action=self.constant_action))
        for _ in range(len(simulation.history)):
            constant_simulation.step()

        constant_df = constant_simulation.history.to_df()
        return simulation.history.to_df()["outcome"] - constant_df["outcome"]
