...
from adaptive_nof1.policies.stabilized_thompson_sampling import (
    StabilizedThompsonSampling,
)
...
def test_stabilization():
    ...
    assert stabilized_thompson_sampling.stabilize_probabilities([0.5, 0.5], 1) == [
        0.5,
        0.5,
    ]
    assert array_almost_equal(
        stabilized_thompson_sampling.stabilize_probabilities([0.8, 0.2], 0.5),
        [0.66666, 0.33333],
        epsilon=0.001,
    )
