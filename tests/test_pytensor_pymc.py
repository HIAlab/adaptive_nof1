...
def three_column_matrix():
    three_column_matrix = pymc.floatX(
...
def three_dimensional_matrix():
    three_dimensional_matrix = pymc.floatX(
...
def test_create_row_from_column_indexes(three_column_matrix, indices):
    ...
    assert list(result) == ([1, 5, 7, 12, 13])
...
def test_model():
    ...
    coefficient_summand = pymc.math.dot(
        coefficients_for_treatment, slopes_for_treatments.T
    ).diagonal()
